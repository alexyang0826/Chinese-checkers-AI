...
player1_set, player2_set = build_sets()
player1_obj, player2_obj = build_obj_sets()
player1_inv_homes, player2_inv_homes = build_invalid_homes_sets()


def alphabeta(board, depth, player, first_player, player1_set, player2_set, alpha, beta):

    board_copy = board[:][:]

    if depth == 0:
        board_score = calculate_board_score(first_player, player1_set, player2_set)
        return board_score, None

    set_pieces = assign_set(player, player1_set, player2_set)

    obj_set = assign_obj_set(player, player1_obj, player2_obj)

    inv_homes_set = assign_invalid_homes_set(player, player1_inv_homes, player2_inv_homes)
    ...
    if player == first_player:
        ...
        for move in valid_moves:
            ...
            player1_set, player2_set = update_player_set(new_set_pieces, player, player1_set, player2_set)

            next_player = player + 1
            if next_player == 3:
                next_player = 1

            score, something = alphabeta(new_board, depth - 1, next_player, first_player, player1_set, player2_set, alpha, beta)
...
def calculate_board_score(player_turn, p1_pieces, p2_pieces):
    ...
    score = calculate_score(player_turn, p1_avg_distance, p2_avg_distance)
...
def calculate_score(player_turn, p1_avg_distance, p2_avg_distance):
    score = 0

    if player_turn == 1:
        # print("-- loop player 1")
        pturn_avg_distance = p1_avg_distance
        score = p2_avg_distance - p1_avg_distance
    elif player_turn == 2:
        # print("-- loop player 2")
        score = p1_avg_distance - p2_avg_distance
    return score
